(function() {
  ...
  function filterButtonFor(type) {
    return Array.from(filters.querySelectorAll('button[data-type]')).find(function(button) {
  ...
  if (filterButtonFor(requestedType)) activeType = requestedType;
  ...
  fetch(root.dataset.indexUrl)
    ...
    .then((data) => {
      ...
      updateCounts();
      const selected = filterButtonFor(activeType) || filterButtonFor('');
      selected.click();
    })
    .catch(() => {
